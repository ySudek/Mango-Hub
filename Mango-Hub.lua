...
-- FixMob helper
local function FixMob(mob, humanoid, hrp)
    pcall(function()
...
function AutoFarmLoop()
    while getgenv().AutoFarmEnabled do
        ...
        pcall(function()
            ...
            -- chưa có quest hoặc sai quest => nhận lại
                ...
                for _, mob in ipairs(Enemies:GetChildren()) do
                    if mob and mob.Name == Mon then
                        ...
                        if humanoid and hrp and humanoid.Health > 0 then
                            -- Move tới mob (luôn trên đầu 30 studs)
                            SmartTopos(hrp.CFrame * CFrame.new(0, 30, 0))

                            -- bật fly nếu chưa bật
                            if not flying then Fly(60) end

                            repeat
                                task.wait(0.08)

                                -- luôn giữ trên đầu mob
                                local myHRP = HRP()
                                if myHRP then
...
function Fly()
    ...
    flying = true
    local hrp = HRP()
    bp = Instance.new("BodyPosition"); bp.MaxForce = Vector3.new(1e6,1e6,1e6); bp.Parent = hrp
